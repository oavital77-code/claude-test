-- מאפשר הזמנת כרטיסייה עם התחלה בעבר (עד חודש אחורה), בשני תרחישים:
-- (א) "הארכה באותו רגע" — המפגש גלש, ורוצים לתפוס את המשבצת הבאה שהתחילה
--     כבר לפני כמה דקות; (ב) "רישום רטרואקטיבי" — לקוח נכנס לטיפול בלי
--     שנרשם במערכת מראש, ומשלימים את הרישום בדיעבד.
--
-- לא נבנה RPC נפרד: אותה בדיקת חפיפה, יתרה ופיקדון (FOR UPDATE, FIFO)
-- שכבר קיימת ב-create_booking מספיקה בדיוק כמו שהיא — משבצת שכבר תפוסה
-- (בעבר או בעתיד) עדיין נחסמת ע"י ה-EXCLUDE constraint על bookings.
-- שינוי יחיד: התחלה מותרת עד 30 יום אחורה במקום רק בעתיד.
--
-- זה חל *רק* על הזמנת כרטיסייה (source='punch_card') — create_booking הוא
-- ה-RPC היחיד שיוצר אותן. הזמנות ססיה נוצרות אך ורק ע"י
-- materialize_session_bookings ולא עוברות דרך כאן, אז חוק הברזל (ססיה לא
-- ניתנת לביטול/שינוי ע"י מטפל) לא נוגע לפונקציה הזו.
