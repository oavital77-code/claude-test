-- בקליניקה — מנוע הזמנות (M4)
-- ר' baclinica-spec.md §6.1-6.2, §3.4, §3.6-3.7, נספח ב'.
-- כל הכתיבה ל-bookings/punch_cards עוברת דרך הפונקציות האלה (SECURITY DEFINER,
-- טרנזקציה יחידה) — ר' CLAUDE.md חוק ברזל #1-#2. שליחת מייל אישור נשארת ל-M7.
...
begin
  ...
  if not found then
    select exists (
      ...
      where user_id = v_uid and active and expires_at > p_ends_at and hours_remaining >= v_hours
    ) into v_has_hours;
  ...
  update punch_cards set hours_remaining = hours_remaining - v_hours where id = v_card.id;
...
begin
  ...
  -- 🔴 מפגש ססיה — לעולם לא ניתן לביטול ע"י המטפל (CLAUDE.md חוק ברזל #4)
  ...
  from app_settings where key = 'cancel_window_hours';
  ...
  insert into audit_log (actor_id, action, entity, entity_id, after)
  values (v_uid, 'booking_cancelled', 'bookings', p_booking_id,
