-- תיקון באג קריטי ב-create_booking: הפונקציה מכריזה RETURNS TABLE עם עמודה
-- בשם hours_remaining, מה שהופך אותו למשתנה plpgsql בהיקף הפונקציה. ה-UPDATE
-- על punch_cards השתמש ב-hours_remaining בלי הכשרה (unqualified), וזה התנגש
-- עם המשתנה — Postgres זרק "column reference hours_remaining is ambiguous"
-- על כל הזמנה בפועל (ר' plpgsql.variable_conflict, ברירת המחדל 'error').
-- כלומר create_booking נכשל תמיד מאז ה-M4 המקורי. מכשירים את שם הטבלה במפורש.
